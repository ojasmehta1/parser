 ...
 *******************************************************************)
let rec left (db:memory) (used: memory) : string =
	match db with
	| h :: t -> if mem h used then left t used else h ^ " " ^ (left t used)
	| _ -> "";;
	
let rec translate (ast:ast_sl) 
	:  string *  string = 
	 let tpl1, tpl2, tpl3 = translate_sl ast [] [] in
	  (" Warning: following variables declared by unused: "^(left tpl1 tpl2) ^ "\n", helper^tpl3^"}\n")
